...
function PlateImage({ model, images }) {
  return (
        ...
        <section className={styles.grid}>
          {sections.slice(0, 6).map((section, sIndex) => (
            <article key={sIndex} className={styles.card}>
              <PlateImage model={section?.model} images={section?.images} />

              {section?.data?.map((row, rIndex) => (
                <div key={rIndex} className={styles.meta} onClick={() => onClickModel({data: section, config: config})}>
