...
const TemplateEditor = lazy(() => import("../templates/posters/posterEditor.jsx"));

import { UpdatePosterTemplate } from "../utils/updateTemplate.js";
...
export default function PosterTemplateWraper() {
  ...
  useEffect(() => {
    const fetchTemplate = async () => {
      try {
        setLoading(true);
        const res = await http.get(`/poster/${type}/code/${code}/template/${id}`);
  ...
  const key = template ? `../templates/posters/${template?.code}.jsx` : null;
  ...
  const handleUpdateMenuTemplate = async (data) => {
    ...
    try{
      setLoading(true);
      const update = await UpdatePosterTemplate(data);
      if(update){
        setTemplate(data);
